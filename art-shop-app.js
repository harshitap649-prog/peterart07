...
app.post('/admin/new-art', requireAdmin, upload.array('images', 3), async (req, res) => {
  const { title, description, price } = req.body;
  if (!req.files || req.files.length === 0) return res.render('admin-new-art', { error: 'At least one image is required' });
  if (req.files.length > 3) return res.render('admin-new-art', { error: 'Maximum 3 images allowed' });
  ...
  try {
    ...
    stmt.run([title.trim(), description ? description.trim() : '', parseFloat(price), primaryImagePath], function (err) {
      ...
      imagePaths.forEach((imgPath, index) => {
        imageStmt.run([artworkId, imgPath, index]);
      });
      imageStmt.finalize();
...
.login-page { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; min-height: 100vh; background: #ffffff; width: 100%; display: flex; flex-direction: column; }
...
.login-card { background: linear-gradient(135deg, #f9fafb 0%, #ffffff 100%); border-radius: 24px; padding: 48px 40px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.2); border: 1px solid #e5e7eb; position: relative; z-index: 10; }
...
.login-button { width: 100%; padding: 14px 24px; font-size: 16px; font-weight: 600; color: #ffffff; background: #000000; border: 1px solid rgba(0, 0, 0, 0.3); border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.3s ease; box-shadow: 0 4px 14px rgba(0, 0, 0, 0.4); position: relative; z-index: 10; pointer-events: auto; }
.login-button:hover { background: #1a1a1a; transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0, 0, 0, 0.6); border-color: rgba(0, 0, 0, 0.5); }
...
.login-banner-ad { text-align: center; padding: 20px; background: #f9fafb; border-top: 1px solid #e5e7eb; margin-top: 40px; position: relative; z-index: 1; min-height: 90px; display: flex; align-items: center; justify-content: center; width: 100%; clear: both; visibility: visible !important; opacity: 1 !important; }
@media (max-width: 768px) { .login-wrapper { padding: 16px; min-height: calc(100vh - 100px); } .login-container { margin-bottom: 24px; } .login-banner-ad { margin-top: 24px; padding: 16px; min-height: 80px; } }
@media (max-width: 480px) { .login-card { padding: 32px 24px; } .login-header h1 { font-size: 24px; } .login-wrapper { padding: 12px; } .login-container { margin-bottom: 20px; } .login-banner-ad { margin-top: 20px; padding: 12px; min-height: 70px; } }`;
